...
public class JavaApplication1 extends Application
{
    ...
    public void start (Stage primaryStage) throws FileNotFoundException
  {
    ...
    button1.setOnAction ((ActionEvent event)->
			 ...
			 option1layout.getChildren ().
			 add (submitButtonOption1);
    ...
    button2.setOnAction ((ActionEvent event)->
			 ...
			 option2InstructorName =
			 new Label ("Enter the instructor name: ");
			 option2InstructorDepartment =
			 new Label ("Enter the affiliated department name: ");
			 option2InstructorID =
			 new Label ("Enter the instructor ID: ");
			 option2layout.add (option2InstructorID, 1, 1);
			 option2layout.add (option2InstructorName, 1, 2);
			 option2layout.add (option2InstructorDepartment, 1,
					    3);
    ...
    submitButtonOption1.setOnAction (new EventHandler < ActionEvent > ()
				     ...
				     new
				     File
				     ("C:\\Users\\kdorji01\\Desktop\\instructor.txt");
				     Scanner instructorDatabase =
				     new Scanner (instructorFile);
				     File instructorFileSize =
				     new
				     File
				     ("C:\\Users\\kdorji01\\Desktop\\instructor.txt");
				     Scanner instructorDatabaseSize =
				     new Scanner (instructorFileSize);
				     File departmentFile =
				     new
				     File
				     ("C:\\Users\\kdorji01\\Desktop\\department.txt");
				     Scanner departmentDatabase =
				     new Scanner (departmentFile);
				     File departmentFileSize =
				     new
				     File
				     ("C:\\Users\\kdorji01\\Desktop\\department.txt");
				     Scanner departmentDatabaseSize =
				     new Scanner (departmentFile);
				     int ID = Integer.parseInt (strID);
				     int arrOfDeptSize = 0;
				     while (departmentDatabaseSize.
					    hasNextLine ())
				     {
				     arrOfDeptSize++;
				     ...
				     new ArrayList <> ();
				     ArrayList < String > instructorLocation =
				     new ArrayList <> ();
				     int arrOfInstSize = 0;
				     while (instructorDatabaseSize.
					    hasNextLine ())
				     {
				     arrOfInstSize++;
				     ...
				     {
				     String line =
				     instructorDatabase.nextLine ();
				     String[]instrucField = line.split (",");
				     instructorID.add (Integer.
						       parseInt (instrucField
								 [0].
								 trim ()));
				     instructorName.add (instrucField[1]);
				     instructorDepartment.
				     add (instrucField[2]); for (int a = 0;
								 a <
								 arrOfDeptSize;
								 a++)
				     {
				     if (instructorDepartment.get (j).
					 equals (departmentName.get (a)))
				     {
				     instructorLocation.add (j,
							     departmentLocation.
							     get (a));}
				     }
				     }
				     ...
				     {
				     option1InstructorName =
				     new Label ("Instructor Name: " +
						instructorName.get (i));
				     option1InstructorDepartment =
				     new Label ("Instructor Department: " +
						instructorDepartment.get (i));
				     option1InstructorLocation =
				     new Label ("Department Location: " +
				     ...
				     setTranslateX (0);
				     option1InstructorLocation.
				     setTranslateY (0); removeButtons ();
				     option1layout.getChildren ().
				     add (option1InstructorName);
				     option1layout.getChildren ().
				     add (option1InstructorDepartment);
				     option1layout.getChildren ().
				     add (option1InstructorLocation);
				     option1flag = true;}
				     }
				     ...
				     {
				     Logger.getLogger (JavaApplication1.class.
						       getName ()).log (Level.
									SEVERE,
									null,
									ex);}
    ...
    submitButtonOption2.setOnAction ((ActionEvent event)->
				     ...
				     new Scanner (departmentFile);
				     FileWriter fw =
				     new FileWriter (instructorFile, true);
				     PrintWriter output =
				     new PrintWriter (fw);
				     int arrOfDeptSize = 0;
				     while (departmentDatabaseSize.
				     ...
				     new
				     Label
				     ("ID entered already exists in the file.");
				     errorlayout.getChildren ().add (error1);
				     error1.setTranslateX (0);
				     error1.setTranslateY (-50);
				     errorlayout.getChildren ().
				     ...
				     {
				     error1 =
				     new Label ("Invalid department format.");
				     errorlayout.getChildren ().add (error1);
				     error1.setTranslateX (0);
				     error1.setTranslateY (-50);
				     errorlayout.getChildren ().
				     ...
				     {
case "MATH":
instructorLocation.add ("RLC"); break; case "BIO":
instructorLocation.add ("LEO"); break; default:
				     instructorLocation.add ("RLC"); break;}
				     }
				     }
				     output.println ();
				     output.print (Integer.parseInt (strID) + "," + strName + "," + strDept); output.close (); Label alertWindow = new Label ("The new instructor has been
added."); errorlayout.getChildren ().add (alertWindow); alertWindow.setTranslateX (0);
				     alertWindow.setTranslateY (-50); errorlayout.getChildren ().add (returnButton); primaryStage.setScene (errorScreen); primaryStage.show ();}
				     }
				     }
				     else
				     {
				     error1 = new Label ("Invalid name format. Please press the go back
button and try again."); errorlayout.getChildren ().add (error1);
				     error1.setTranslateX (0); error1.setTranslateY (-50); errorlayout.getChildren ().add (returnButton); primaryStage.setScene (errorScreen);
				     primaryStage.show ();}
				     }
				     }
				     }
